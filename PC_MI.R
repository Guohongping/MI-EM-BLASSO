PC_MI<-function(phen,gene,ps,svpal){
  ...
  ppp<-matrix(0,ncol(X1)*1,ncol=1,nrow=ncol(X1))
  for (i in 1:k)
  {
  ...
  Res1<-as.vector(Res)	
  pp1<- t(as.matrix((rowSums(ppp)/ncol(ppp))))
  pp1<-as.vector(pp1)	
  
  
  kk<-length(which(abs(Res1)>1e-5&pp1<svpal)) 
  
  if(kk!=0){
    
    sig1<-which(abs(Res1)>1e-5&pp1<svpal)
    bbo<-matrix(0,kk,1)
    for (i in 1:kk){
      bbo[i,]=Res1[sig1[i]]
    }
